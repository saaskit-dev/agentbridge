...
describe('ConsoleSink', () => {
  ...
  it('includes trace and data in compact format', () => {
    ...
    const sink = new ConsoleSink({ color: false });
    sink.write(
      makeEntry({ traceId: 'abc12345-long', sessionId: 'sess1234-long', data: { count: 42 } })
    );
...
describe('FileSink', () => {
  ...
  it('deletes oldest files when maxFiles is exceeded (RFC §5.3)', () => {
    ...
    const prefixes = [
      'maxfiles-2026-01-01-00',
      'maxfiles-2026-01-01-01',
      'maxfiles-2026-01-01-02',
      'maxfiles-2026-01-01-03',
      'maxfiles-2026-01-01-04',
    ];
    ...
    const files = readdirSync(testDir).filter(
      f => f.startsWith('maxfiles') && f.endsWith('.jsonl')
    );
...
describe('RemoteSink', () => {
  it('buffers entries and sanitizes before upload', async () => {
    ...
    const body = JSON.parse(opts.body) as Array<{
      common: unknown;
  ...
  it('respects minLevel', async () => {
    ...
    const body = JSON.parse(mockFetch.mock.calls[0][1].body) as Array<{
      common: unknown;
  ...
  it('applies extraSanitizer after built-in sanitizer (RFC §6.3)', async () => {
    ...
    const extraSanitizer = new Sanitizer({
      extraSensitiveKeys: ['sessionId'],
      maxStringLength: 10,
    });
    ...
    sink.write(
      makeEntry({ data: { sessionId: 'sess-abc', longValue: 'a'.repeat(20), safe: 'ok' } })
    );
...
describe('NewRelicBackend', () => {
  it('builds correct request format', () => {
    ...
    const req = backend.buildRequest([makeEntry({ message: 'hello', traceId: 'trace-1' })], {
      deviceId: 'dev-1',
      appVersion: '1.0',
      layer: 'server',
    })!;
  ...
  it('omits undefined optional attributes from payload', () => {
    ...
    const req = backend.buildRequest([makeEntry()], {
      deviceId: 'dev-1',
      appVersion: '1.0',
      layer: 'cli',
    })!;
  ...
  it('includes optional attributes when present', () => {
    const backend = new NewRelicBackend({ licenseKey: 'key' });
    const req = backend.buildRequest([makeEntry({ traceId: 'tid', durationMs: 42 })], {
      deviceId: 'dev-1',
      appVersion: '1.0',
      layer: 'cli',
      machineId: 'machine-1',
...
describe('ServerRelayBackend', () => {
  ...
  it('builds correct request format with string token', () => {
    const backend = new ServerRelayBackend({
...
describe('createRemoteBackend / setTelemetryToken (RFC §24.2)', () => {
  ...
  it('setTelemetryToken enables backend to produce requests', async () => {
    ...
    const noTokenReq = backend.buildRequest([makeEntry()], {
      deviceId: 'dev',
      appVersion: '1.0',
      layer: 'test',
    });
...
describe('AxiomBackend (RFC §23.3)', () => {
  ...
  it('builds correct request format for Axiom ingest', () => {
    ...
    const req = backend.buildRequest([makeEntry({ message: 'hello', traceId: 'trace-1' })], meta)!;
