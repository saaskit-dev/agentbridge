...
describe('Sanitizer', () => {
  ...
  describe('sensitive key redaction', () => {
    it('redacts known sensitive keys', () => {
      const result = sanitizer.process(makeEntry({
        token: 'abc123',
        apiKey: 'secret',
        password: 'hunter2',
        safe: 'visible',
  ...
  describe('error sanitization', () => {
    it('preserves error stacks', () => {
      const result = sanitizer.process(makeEntry(undefined, {
    ...
    it('redacts key=value and password: value patterns in error messages', () => {
      const r1 = sanitizer.process(makeEntry(undefined, { message: 'invalid key=supersecret supplied' }))
      expect(r1.error!.message).not.toContain('supersecret')

      const r2 = sanitizer.process(makeEntry(undefined, { message: 'bad password: "hunter2" rejected' }))
