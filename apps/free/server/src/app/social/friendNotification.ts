...
export async function sendFriendRequestNotification(
  ...
  if (
    ...
    !shouldSendNotification(receiverRelationship.lastNotifiedAt, receiverRelationship.status)
  ) {
    return;
  }
...
export async function sendFriendshipEstablishedNotification(
  ...
  if (
    user1Relationship &&
    shouldSendNotification(user1Relationship.lastNotifiedAt, user1Relationship.status)
  ) {
  ...
  if (
    user2Relationship &&
    shouldSendNotification(user2Relationship.lastNotifiedAt, user2Relationship.status)
  ) {
