...
const { state, emitUpdateMock, dbMock, resetState, seedSession, seedMessage } = vi.hoisted(() => {
  const state = {
  ...
  const sessionFindFirst = vi.fn(async (args: any) => {
    const row = state.sessions.find(
      session => session.id === args?.where?.id && session.accountId === args?.where?.accountId
    );
    if (!row) {
      return null;
    }
