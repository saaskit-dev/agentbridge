...
export async function startSocket(app: Fastify) {
    ...
    if (process.env.DATABASE_URL && !process.env.PGLITE_DIR) {
        try {
            ...
            log({ module: 'websocket', level: 'error' }, `Failed to initialize PostgreSQL adapter: ${error}`);
        }
    } else {
        log({ module: 'websocket' }, 'Running in single-instance mode (PGlite or no external PostgreSQL)');
    }

    let rpcListeners = new Map<string, Map<string, Socket>>();
    io.on("connection", async (socket) => {
        log({ module: 'websocket', level: 'debug' }, `New connection attempt from socket: ${socket.id}`);
