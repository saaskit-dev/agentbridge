...
export async function fetchRepeatKey(tx: Tx, key: string) {
  const session = await tx.repeatKey.findUnique({ where: { key, expiresAt: { gte: new Date() } } });
  if (session) {
...
export async function saveRepeatKey(
  ...
  key: string,
  value: string,
  timeout: number = Date.now() + 1000 * 60 * 60 * 24 /* 1 day */
) {
...
export async function repeatKey(
  ...
  const session = await tx.repeatKey.findUnique({ where: { key, expiresAt: { lte: new Date() } } });
  if (session) {
    return false;
  }
  await tx.repeatKey.upsert({
