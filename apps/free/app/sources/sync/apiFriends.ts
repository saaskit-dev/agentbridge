...
export async function getUserProfile(
  ...
  return await backoff(async () => {
    const response = await fetch(`${API_ENDPOINT}/v1/user/${userId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${credentials.token}`,
      },
...
export async function getUserProfiles(
  ...
  const profiles = await Promise.all(userIds.map(id => getUserProfile(credentials, id)));
 ...
 */
export async function getFriendsList(credentials: AuthCredentials): Promise<UserProfile[]> {
  const API_ENDPOINT = getServerUrl();
