...
class ProfileSyncService {
  ...
  /**
   * Sync profiles from GUI to CLI using proper Free infrastructure
   * SECURITY NOTE: Direct file access is PROHIBITED - use Free RPC infrastructure
   */
  public async syncGuiToCli(profiles: AIBackendProfile[]): Promise<void> {
    ...
    try {
      ...
      this.emitEvent({
        ...
        timestamp: Date.now(),
        message: 'Profiles available through Free settings system',
      });
    } catch (error) {
  ...
  /**
   * Sync profiles from CLI to GUI using proper Free infrastructure
   * SECURITY NOTE: Direct file access is PROHIBITED - use Free RPC infrastructure
   */
  public async syncCliToGui(): Promise<AIBackendProfile[]> {
    ...
    try {
      // CLI profiles are accessed through Free settings system, not direct file access
      ...
      this.emitEvent({
        ...
        timestamp: Date.now(),
        message: 'Profiles retrieved from Free settings system',
      });
    ...
    for (const guiProfile of guiProfiles) {
      ...
      if (cliProfile) {
        ...
        switch (conflictResolution) {
          ...
          case 'most-recent':
            resolvedProfile =
              guiProfile.updatedAt! >= cliProfile.updatedAt!
                ? { ...guiProfile }
                : { ...cliProfile };
            break;
          case 'merge':
  ...
  /**
   * Set active profile using Free settings infrastructure
   * SECURITY NOTE: Direct file access is PROHIBITED - use Free settings system
   */
  ...
  /**
   * Get active profile using Free settings infrastructure
   * SECURITY NOTE: Direct file access is PROHIBITED - use Free settings system
   */
  public async getActiveProfile(): Promise<AIBackendProfile | null> {
    try {
      // Get active profile from Free settings system
...
export const syncGuiToCli = (profiles: AIBackendProfile[]) =>
  profileSyncService.syncGuiToCli(profiles);
export const syncCliToGui = () => profileSyncService.syncCliToGui();
export const bidirectionalSync = (guiProfiles: AIBackendProfile[]) =>
  profileSyncService.bidirectionalSync(guiProfiles);
export const setActiveProfile = (profileId: string) =>
  profileSyncService.setActiveProfile(profileId);
