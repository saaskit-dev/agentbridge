...
const createToolCall = (
  ...
  completedAt:
    state === 'completed' || state === 'error' ? Date.now() - Math.random() * 5000 : null,
  description: description || null,
  result,
});

// Reusable Read tool call constant
const createReadToolCall = (
  id: string,
  filePath: string,
  startLine: number,
  endLine: number,
...
This function handles validation, transformation, and normalization in a single pass.`,
  ...
  createSectionTitle(
    'missing-tool-call-title',
    'What happens when a tool call Message has zero tools? If the empty tools array would render anything, it would show up between these two messages\nvvvvvvvvvvvvvvvvvvvv'
  ),
    ...
    tool: createToolCall(
      ...
      'completed',
      {
        todos: [
          {
            id: '1',
            content: 'Fix TypeScript errors in Button component',
            status: 'completed',
            priority: 'high',
          },
          {
            id: '2',
            content: 'Add error boundary to App component',
            status: 'in_progress',
            priority: 'medium',
          },
          ...
          {
            id: '4',
            content: 'Add input validation to helpers',
            status: 'pending',
            priority: 'low',
          },
        ],
