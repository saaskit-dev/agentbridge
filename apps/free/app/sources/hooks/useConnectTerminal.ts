...
export function useConnectTerminal(options?: UseConnectTerminalOptions) {
    ...
    const createAccountAndConnect = React.useCallback(async (url: string) => {
        if (!url.startsWith('free://terminal?')) {
    ...
    const processAuthUrl = React.useCallback(async (url: string) => {
        ...
        if (!auth.credentials) {
            Modal.alert(t('common.error'), 'Please login first', [{ text: t('common.ok') }]);
            return false;
        }
    ...
    React.useEffect(() => {
        if (CameraView.isModernBarcodeScannerAvailable) {
            const subscription = CameraView.onModernBarcodeScanned(async (event) => {
                if (event.data.startsWith('free://terminal?')) {
