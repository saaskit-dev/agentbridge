...
describe('extractEnvVarReferences', () => {
  ...
  it('handles DeepSeek profile pattern', () => {
    ...
    expect(extractEnvVarReferences(envVars).sort()).toEqual([
      'DEEPSEEK_AUTH_TOKEN',
      'DEEPSEEK_BASE_URL',
    ]);
  });
...
describe('resolveEnvVarSubstitution', () => {
  ...
  it('resolves ${VAR:-default} when VAR present', () => {
    expect(resolveEnvVarSubstitution('${BASE_URL:-https://default.com}', daemonEnv)).toBe(
  ...
  it('returns literal URL for non-substitution', () => {
    expect(resolveEnvVarSubstitution('https://api.example.com', daemonEnv)).toBe(
  ...
  it('resolves DeepSeek default URL pattern', () => {
    expect(
      resolveEnvVarSubstitution('${DEEPSEEK_BASE_URL:-https://api.deepseek.com/anthropic}', {})
  ...
  it('resolves actual value over default when present', () => {
    ...
    expect(
      resolveEnvVarSubstitution('${DEEPSEEK_BASE_URL:-https://api.deepseek.com/anthropic}', env)
    ).toBe('https://custom.deepseek.com');
  });

  it('handles complex default values with special characters', () => {
    expect(
      resolveEnvVarSubstitution('${URL:-https://api.example.com/v1?key=value&foo=bar}', {})
