...
function SessionInfoContent({ session }: { session: Session }) {
  ...
  const isCliOutdated =
    session.metadata?.version && !isVersionSupported(session.metadata.version, MINIMUM_CLI_VERSION);
  ...
  const handleCopyUpdateCommand = useCallback(async () => {
    const updateCommand = 'npm install -g @free/cli@latest';
    try {
      ...
      <ItemList>
        ...
        {session.metadata && (
          <ItemGroup title={t('sessionInfo.metadata')}>
            ...
            {session.metadata.freeHomeDir && (
              <Item
                title={t('sessionInfo.freeHome')}
        ...
        {devModeEnabled && (
          <ItemGroup title="Raw JSON (Dev Mode)">
            {session.agentState && (
              <>
                ...
                <View style={{ marginHorizontal: 16, marginBottom: 12 }}>
                  <CodeView code={JSON.stringify(session.agentState, null, 2)} language="json" />
                </View>
              </>
            )}
            {session.metadata && (
              <>
                ...
                />
                <View style={{ marginHorizontal: 16, marginBottom: 12 }}>
                  <CodeView code={JSON.stringify(session.metadata, null, 2)} language="json" />
                </View>
              </>
            )}
            {sessionStatus && (
            ...
            />
            <View style={{ marginHorizontal: 16, marginBottom: 12 }}>
              <CodeView code={JSON.stringify(session, null, 2)} language="json" />
            </View>
          </ItemGroup>
        )}
      </ItemList>
