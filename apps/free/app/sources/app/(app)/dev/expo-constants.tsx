    ...
    <View style={{ marginBottom: 12 }}>
      ...
      >
        <Ionicons
          ...
          style={{ marginRight: 8 }}
        />
        <Text style={{ flex: 1, fontSize: 16, ...Typography.default('semiBold') }}>{title}</Text>
    ...
    <>
      ...
      <ItemList>
        ...
        <ItemGroup title="Configuration from Constants API">
          <JsonViewer title="expoConfig (Current)" data={expoConfig} defaultExpanded={true} />
          <JsonViewer title="manifest (Legacy)" data={manifest} />
          <JsonViewer title="manifest2" data={manifest2} />
          {updatesManifest && <JsonViewer title="Updates.manifest" data={updatesManifest} />}
        </ItemGroup>
        ...
        <ItemGroup title="Raw Native Module Manifests">
          ...
          {parsedUpdatesManifest && (
            <JsonViewer title="ExpoUpdates.manifest (OTA)" data={parsedUpdatesManifest} />
          )}
          {parsedDevLauncherManifest && (
            <JsonViewer title="DevLauncher.manifest" data={parsedDevLauncherManifest} />
          )}
        </ItemGroup>
        ...
        <ItemGroup title="System Constants">
          <Item
            ...
            showChevron={false}
          />
          <Item title="Session ID" detail={Constants.sessionId} showChevron={false} />
          <Item title="Installation ID" detail={Constants.installationId} showChevron={false} />
          <Item title="Is Device" detail={Constants.isDevice ? 'Yes' : 'No'} showChevron={false} />
          <Item
            title="Debug Mode"
        ...
        <ItemGroup title="Updates Information">
          <Item title="Update ID" detail={updateId || 'Not available'} showChevron={false} />
          <Item
            title="Release Channel"
            detail={releaseChannel || 'Not available'}
            showChevron={false}
          />
          <Item title="Channel" detail={channel || 'Not available'} showChevron={false} />
          <Item
            title="Is Embedded Launch"
            detail={
              isEmbeddedLaunch !== undefined ? (isEmbeddedLaunch ? 'Yes' : 'No') : 'Not available'
            }
            showChevron={false}
          />
        </ItemGroup>
        ...
        <ItemGroup title="Native Modules">
          ...
          />
          {ExponentConstants && (
            <JsonViewer title="ExponentConstants (full module)" data={ExponentConstants} />
          )}
          {ExpoUpdates && <JsonViewer title="ExpoUpdates (full module)" data={ExpoUpdates} />}
        </ItemGroup>
