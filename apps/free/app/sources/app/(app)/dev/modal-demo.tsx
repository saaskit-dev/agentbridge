  ...
  return (
    <View style={styles.customModal}>
      ...
      <View style={styles.customModalButtons}>
        <RoundButton title="Close" onPress={onClose} size="normal" />
      </View>
    </View>
  );
}
...
export default function ModalDemoScreen() {
  ...
  const showAlertWithButtons = () => {
    Modal.alert('Multiple Actions', 'Choose an action:', [
  ...
  const showConfirm = async () => {
    const result = await Modal.confirm('Confirm Action', 'Are you sure you want to proceed?');
      ...
      <ItemList>
        ...
        <ItemGroup title="Last Action Result">
          <View style={styles.resultContainer}>
            <Text style={[styles.resultText, Typography.default()]}>{lastResult}</Text>
          </View>
        </ItemGroup>
      </ItemList>
    </ScrollView>
