...
npm ERR!     npm run start`,
    ...
    bashLongCommand: {
      ...
      input: {
        command:
          'git log --pretty=format:"%h - %an, %ar : %s" --graph --since=2.weeks --author="John Doe" --grep="fix" --all',
...
* 5b4c3d2 - John Doe, 10 days ago : fix: update dependencies to patch vulnerabilities`,
    ...
    bashMultiline: {
      ...
      input: {
        command:
          'echo "Starting deployment..." && npm run build && npm run test && echo "Deployment complete!"',
      ...
      result: JSON.stringify(
        {
          results: [
            { file: 'App.tsx', line: 5, match: 'const [count, setCount] = useState(0);' },
            {
              ...
              line: 3,
              match: 'const [value, setValue] = useState(props.initial);',
            },
          ],
          totalMatches: 2,
        },
            ...
            {selectedExample === 'status' ? (
              <>
                ...
                <View style={styles.statusSection}>
                  ...
                  {renderExample('bashError', {
                    ...examples.bashError,
                    name: 'Error State (Warning Icon)',
                  })}
                  {renderExample('toolDenied', {
                    ...examples.toolDenied,
                    name: 'Denied State (Neutral Icon)',
                  })}
                  {renderExample('toolCanceled', {
                    ...examples.toolCanceled,
                    name: 'Canceled State (Neutral Icon)',
                  })}
                </View>
              </>
            ) : null}
