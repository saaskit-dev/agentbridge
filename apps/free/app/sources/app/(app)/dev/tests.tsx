...
export default function TestsScreen() {
  ...
  const runSuite = async (suiteName: string) => {
    ...
    try {
      ...
      if (result) {
        setState(prev => ({
          running: false,
          results: [...prev.results.filter(r => r.name !== suiteName), result],
        }));
      }
    } catch (error) {
  ...
  const passedTests = state.results.reduce(
            ...
            <View style={{ flex: 1 }}>
              <Text
                style={{
                  ...
                  fontSize: 32,
                  fontWeight: '600',
                  color: failedTests > 0 ? '#FF3B30' : '#8E8E93',
                }}
              >
                {failedTests}
              </Text>
          ...
          {suite.tests.map((test, index) => (
            <View key={index} style={{ backgroundColor: 'white' }}>
              ...
              >
                ...
                />
                <View style={{ flex: 1 }}>
                  <Text style={{ ...Typography.default(), fontSize: 16 }}>{test.name}</Text>
                  <Text
                    style={{ ...Typography.mono(), fontSize: 12, color: '#8E8E93', marginTop: 2 }}
                  >
                    {test.duration}ms
                  </Text>
                </View>
