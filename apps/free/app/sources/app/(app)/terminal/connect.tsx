...
export default function TerminalConnectScreen() {
    ...
    const { processAuthUrl, createAccountAndConnect, isLoading, isCreatingAccount } = useConnectTerminal({
        onSuccess: () => {
            router.back();
        }
    });
    ...
    const isProcessing = isLoading || isCreatingAccount;
    ...
    const handleConnect = async () => {
        if (publicKey) {
            // Convert the hash key format to the expected free:// URL format
            const authUrl = `free://terminal?${publicKey}`;
            if (isAuthenticated) {
                await processAuthUrl(authUrl);
            } else {
                await createAccountAndConnect(authUrl);
            }
        }
    };
    ...
    const getButtonText = () => {
        if (isCreatingAccount) {
            return t('terminal.creatingAccount');
        }
        if (isLoading) {
            return t('terminal.connecting');
        }
        return isAuthenticated ? t('terminal.acceptConnection') : t('terminal.createAccountAndAccept');
    };
