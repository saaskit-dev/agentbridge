...
export default function TerminalScreen() {
  ...
  const handleConnect = async () => {
    if (publicKey) {
      // Use the full free:// URL format expected by the hook
  ...
  return (
    <>
      <ItemList>
        ...
        {/* Security Notice */}
        <ItemGroup title={t('terminal.security')} footer={t('terminal.securityFooterDevice')}>
          <Item
            title={t('terminal.clientSideProcessing')}
            subtitle={t('terminal.linkProcessedOnDevice')}
            icon={
              <Ionicons name="shield-checkmark-outline" size={29} color={theme.colors.success} />
            }
            showChevron={false}
          />
        </ItemGroup>
