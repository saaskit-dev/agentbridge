...
export default function ArtifactsScreen() {
  ...
  const renderItem = React.useCallback(
    ({ item, index }: { item: DecryptedArtifact; index: number }) => {
      ...
      return (
        <Pressable
          style={[
            styles.artifactItem,
            isSingle
              ? styles.artifactItemSingle
              : isFirst
                ? styles.artifactItemFirst
                : isLast
                  ? styles.artifactItemLast
                  : {},
          ]}
        ...
        >
          <View style={styles.artifactContent}>
            <Text
              style={[styles.artifactTitle, !item.title && styles.artifactUntitled]}
              numberOfLines={1}
            >
              {item.title || 'Untitled'}
            </Text>
    ...
    if (isLoading) {
      return (
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.emptyDescription, { marginTop: 16 }]}>{t('artifacts.loading')}</Text>
        </View>
      );
    }
    ...
    return (
      <View style={styles.emptyContainer}>
        <Ionicons
          ...
          color={theme.colors.textSecondary}
        />
        <Text style={styles.emptyTitle}>{t('artifacts.empty')}</Text>
        <Text style={styles.emptyDescription}>{t('artifacts.emptyDescription')}</Text>
      </View>
    );
  }, [isLoading, styles]);
