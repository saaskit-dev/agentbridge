...
export default function FriendsScreen() {
  ...
  const [acceptLoading, doAccept] = useFreeAction(async () => {
    if (!credentials || !processingId) return;

    const fromUserId = processingId;
    // sendFriendRequest also accepts existing requests
    const updatedProfile = await sendFriendRequest(credentials, fromUserId);
  ...
  const [rejectLoading, doReject] = useFreeAction(async () => {
    if (!credentials || !processingId) return;
  ...
  const [removeLoading, doRemove] = useFreeAction(async () => {
    if (!credentials || !processingId) return;
  ...
  const isProcessing = (id: string) =>
    processingId === id && (acceptLoading || rejectLoading || removeLoading);

  return (
    <ItemList style={{ paddingTop: 0 }}>
      {/* Friend Requests Section */}
      {friendRequests.length > 0 && (
        <ItemGroup title={t('friends.pendingRequests')} style={styles.groupStyle}>
      ...
      {requestedFriends.length > 0 && (
        <ItemGroup title={t('friends.requestPending')} style={styles.groupStyle}>
      ...
      <ItemGroup title={t('friends.myFriends')} style={styles.groupStyle}>
        {friends.length === 0 ? (
