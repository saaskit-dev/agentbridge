  ...
  const content = (
    <>
      <View style={[styles.container, containerPadding, style]}>
        {/* Left Section */}
        {(icon || leftElement) && <View style={styles.iconContainer}>{leftElement || icon}</View>}

        {/* Center Section */}
        <View style={styles.centerContent}>
          <Text style={[styles.title, titleColor, titleStyle]} numberOfLines={subtitle ? 1 : 2}>
            {title}
          </Text>
          {subtitle &&
            (() => {
              ...
              const effectiveLines =
                subtitleLines !== undefined
                  ...
                  : typeof subtitle === 'string' && subtitle.indexOf('\n') !== -1
                    ...
                    : 1;
              return (
                <Text style={[styles.subtitle, subtitleStyle]} numberOfLines={effectiveLines}>
                  {subtitle}
                </Text>
              );
            })()}
        ...
        <View style={styles.rightSection}>
          {detail && !rightElement && (
            <Text
              style={[styles.detail, { marginRight: showAccessory ? 6 : 0 }, detailStyle]}
              numberOfLines={1}
            >
              {detail}
            </Text>
            ...
            styles.divider,
            {
              marginLeft:
                isAndroid || isWeb
                  ? 0
                  : dividerInset +
                    (icon || leftElement ? 16 + (isIOS && !isWeb ? 29 : 32) + 15 : 16),
            },
          ]}
        />
      )}
