...
function ManualConfigurationItem({
  isSelected,
  onSelect,
  onUseCliVars,
  onConfigureManually,
...
export function NewSessionWizard({
  ...
  const builtInProfiles: AIBackendProfile[] = useMemo(
    () => [
      ...
      },
      {
        ...
        environmentVariables: [{ name: 'AZURE_OPENAI_API_VERSION', value: '2024-02-15-preview' }],
        compatibility: { claude: false, codex: true, gemini: false, opencode: false },
        isBuiltIn: true,
        createdAt: Date.now(),
        updatedAt: Date.now(),
  ...
  const steps: WizardStep[] = React.useMemo(() => {
    ...
    if (profileNeedsConfiguration(selectedProfileId)) {
      ...
      return [...beforeProfile, 'profileConfig', ...afterProfile] as WizardStep[];
    }
  ...
  const getProfileRequiredFields = (
    profileId: string | null
  ): Array<{ key: string; label: string; placeholder: string; isPassword?: boolean }> => {
    if (!profileId) return [];
    const profile = allProfiles.find(p => p.id === profileId);
    if (!profile) return [];
  ...
  const handleDuplicateProfile = (profile: AIBackendProfile) => {
    Modal.prompt('Duplicate Profile', `Enter a name for the duplicate of "${profile.name}":`, {
  ...
  const handleNext = () => {
    ...
    if (
      ...
      (!selectedProfileId || !profileNeedsConfiguration(selectedProfileId))
    ) {
  ...
  const canProceed = useMemo(() => {
    switch (currentStep) {
      ...
      case 'path':
        return (
          selectedPath.trim().length > 0 || (showCustomPathInput && customPath.trim().length > 0)
        );
      case 'prompt':
              ...
              {getProfileRequiredFields(selectedProfileId).map(field => (
                <View key={field.key} style={{ marginBottom: 16 }}>
                  ...
                  <TextInput
                    ...
                    value={
                      (profileApiKeys[selectedProfileId!] as any)?.[field.key] ||
                      (profileConfigs[selectedProfileId!] as any)?.[field.key] ||
                      ''
                    }
                    onChangeText={text => {
                      if (field.isPassword) {
      ...
      case 'agent':
        return (
          <View>
            ...
            <Text style={styles.stepDescription}>Select which AI assistant you want to use</Text>
            ...
            <Pressable
              style={[
                ...
                selectedProfileId &&
                  !allProfiles.find(p => p.id === selectedProfileId)?.compatibility.claude && {
                    opacity: 0.5,
            ...
            >
              ...
              <View style={styles.agentInfo}>
                ...
                {selectedProfileId &&
                  !allProfiles.find(p => p.id === selectedProfileId)?.compatibility.claude && (
                    <Text
                      style={{ fontSize: 12, color: theme.colors.textDestructive, marginTop: 4 }}
                    >
            ...
            <Pressable
              style={[
                ...
                selectedProfileId &&
                  !allProfiles.find(p => p.id === selectedProfileId)?.compatibility.codex && {
                    opacity: 0.5,
                    backgroundColor: theme.colors.surface,
                  },
              ]}
            ...
            >
              ...
              <View style={styles.agentInfo}>
                ...
                <Text style={styles.agentDescription}>OpenAI's specialized coding assistant</Text>
                {selectedProfileId &&
                  !allProfiles.find(p => p.id === selectedProfileId)?.compatibility.codex && (
                    <Text
                      style={{ fontSize: 12, color: theme.colors.textDestructive, marginTop: 4 }}
      ...
      case 'options':
        return (
          <View>
            ...
            <Text style={styles.stepDescription}>Configure how the AI agent should behave</Text>
            ...
            {selectedProfileId && (
              ...
              >
                ...
                >
                  Using profile:{' '}
                  {allProfiles.find(p => p.id === selectedProfileId)?.name || 'Unknown'}
                </Text>
                <Text
                  style={{
                    fontSize: 12,
            ...
            <ItemGroup title="Permission Mode">
              {(
                ...
                <Item
                  ...
                  leftElement={
                    <Ionicons name={option.icon} size={24} color={theme.colors.textSecondary} />
                  }
                  rightElement={
      ...
      case 'machine':
        return (
          <View>
            ...
            <Text style={styles.stepDescription}>Choose which machine to run your session on</Text>
            ...
            <ItemGroup title="Available Machines">
              {machines.map((machine, index) => (
                <Item
                  ...
                  leftElement={
                    <Ionicons name="laptop-outline" size={24} color={theme.colors.textSecondary} />
                  }
                  rightElement={
      ...
      case 'path':
        return (
          <View>
            ...
            <Text style={styles.stepDescription}>Choose the directory to work in</Text>
            ...
            <ItemGroup title="Custom Directory">
              <Item
                ...
                subtitle={
                  showCustomPathInput && customPath ? customPath : 'Specify a custom directory path'
                }
                leftElement={
      ...
      case 'prompt':
        return (
          <View>
            ...
            <Text style={styles.stepDescription}>Write your first message to the AI agent</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        ...
        <Pressable
          style={[styles.button, styles.buttonPrimary, !canProceed && { opacity: 0.5 }]}
          onPress={handleNext}
          disabled={!canProceed}
        >
