...
export const ItemList = React.memo<ItemListProps>(props => {
  ...
  const { children, style, containerStyle, insetGrouped = true, ...scrollViewProps } = props;
...
export const ItemListStatic = React.memo<
  Omit<ItemListProps, keyof ScrollViewProps> & {
    children: React.ReactNode;
    style?: StyleProp<ViewStyle>;
    containerStyle?: StyleProp<ViewStyle>;
    insetGrouped?: boolean;
...
>(props => {
  ...
  const { children, style, containerStyle, insetGrouped = true } = props;
