      ...
      for (let x = 0; x < qrMatrix.size; x++) {
        ...
        if (neighbors.current) {
          ...
          if (tlr || trr || brr || blr) {
            ...
            elements.push(<Path key={`${x}-${y}`} path={path} color={foregroundColor} />);
          } else {
  ...
  return (
    <Canvas style={{ width: size, height: size }}>
      ...
      <Group transform={[{ translateX: moduleSize * 2 }, { translateY: moduleSize * 2 }]}>
        ...
        <DiffRect
          inner={rrect(
            rect(moduleSize, moduleSize, 5 * moduleSize, 5 * moduleSize),
            moduleSize * baseRadius,
            moduleSize * baseRadius
          )}
          outer={rrect(
            rect(0, 0, 7 * moduleSize, 7 * moduleSize),
            moduleSize * (baseRadius + 1),
            moduleSize * (baseRadius + 1)
          )}
          color={foregroundColor}
        />
        ...
        <DiffRect
          inner={rrect(
            rect((qrMatrix.size - 7 + 1) * moduleSize, moduleSize, 5 * moduleSize, 5 * moduleSize),
            moduleSize * baseRadius,
            moduleSize * baseRadius
          )}
          outer={rrect(
            rect((qrMatrix.size - 7) * moduleSize, 0, 7 * moduleSize, 7 * moduleSize),
            moduleSize * (baseRadius + 1),
            moduleSize * (baseRadius + 1)
          )}
          color={foregroundColor}
        ...
        <DiffRect
          inner={rrect(
            rect(moduleSize, (qrMatrix.size - 7 + 1) * moduleSize, 5 * moduleSize, 5 * moduleSize),
            moduleSize * baseRadius,
            moduleSize * baseRadius
          )}
          outer={rrect(
            rect(0, (qrMatrix.size - 7) * moduleSize, 7 * moduleSize, 7 * moduleSize),
            moduleSize * (baseRadius + 1),
            moduleSize * (baseRadius + 1)
          )}
          color={foregroundColor}
