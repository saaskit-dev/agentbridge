...
export const knownTools = {
  ...
  Bash: {
    ...
    extractDescription: (opts: { metadata: Metadata | null; tool: ToolCall }) => {
      if (typeof opts.tool.input.command === 'string') {
        ...
        if (
          ['cd', 'ls', 'pwd', 'mkdir', 'rm', 'cp', 'mv', 'npm', 'yarn', 'git'].includes(firstWord)
        ) {
  ...
  CodexBash: {
    ...
    extractSubtitle: (opts: { metadata: Metadata | null; tool: ToolCall }) => {
      ...
      if (
        opts.tool.input?.parsed_cmd &&
        Array.isArray(opts.tool.input.parsed_cmd) &&
        opts.tool.input.parsed_cmd.length > 0
      ) {
        const parsedCmd = opts.tool.input.parsed_cmd[0];
        if (parsedCmd.cmd) {
  ...
  string,
  {
    ...
    minimal?:
      | boolean
      | ((opts: { metadata: Metadata | null; tool: ToolCall; messages?: Message[] }) => boolean);
