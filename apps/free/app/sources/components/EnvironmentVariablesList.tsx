...
export function EnvironmentVariablesList({
  ...
  const getDocumentation = React.useCallback(
    (varName: string) => {
      if (!profileDocs)
        return { expectedValue: undefined, description: undefined, isSecret: false };
  ...
  const handleDuplicateVariable = React.useCallback(
    (index: number) => {
  ...
  return (
    <View style={{ marginBottom: 16 }}>
      ...
      {environmentVariables.map((envVar, index) => {
        ...
        const isSecret =
          docs.isSecret ||
          /TOKEN|KEY|SECRET|AUTH/i.test(envVar.name) ||
          /TOKEN|KEY|SECRET|AUTH/i.test(varNameFromValue || '');
