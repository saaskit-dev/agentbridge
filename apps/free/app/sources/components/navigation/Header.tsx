    ...
    <View style={[containerStyle]}>
      <View style={styles.contentWrapper}>
        <View style={[styles.content, { height: headerHeight }]}>
          <View style={styles.leftContainer}>{headerLeft && headerLeft()}</View>
...
const DefaultBackButton: React.FC<{ tintColor?: string; onPress: () => void }> = ({
  tintColor = '#000',
  onPress,
}) => {
  return (
...
const NavigationHeaderComponent: React.FC<NativeStackHeaderProps> = React.memo(props => {
  ...
  if (options.headerLeft) {
    ...
    headerLeftContent = () =>
      options.headerLeft!({ canGoBack: !!back, tintColor: options.headerTintColor });
    ...
    headerLeftContent = () => (
      <DefaultBackButton tintColor={options.headerTintColor} onPress={() => navigation.goBack()} />
    );
  }
