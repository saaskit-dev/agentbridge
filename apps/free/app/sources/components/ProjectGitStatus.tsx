...
export function ProjectGitStatus({ sessionId }: ProjectGitStatusProps) {
  ...
  const hasChanges =
    fileCount > 0 || gitStatus.unstagedLinesAdded > 0 || gitStatus.unstagedLinesRemoved > 0;
      ...
      {hasLineChanges && (
        <View style={styles.lineChanges}>
          {gitStatus.unstagedLinesAdded > 0 && (
            <Text style={styles.addedText}>+{gitStatus.unstagedLinesAdded}</Text>
          )}
          {gitStatus.unstagedLinesRemoved > 0 && (
            <Text style={styles.removedText}>-{gitStatus.unstagedLinesRemoved}</Text>
          )}
        </View>
      )}
    </View>
