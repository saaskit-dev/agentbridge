# Free CLI Codebase Overview

## Project Overview

Free CLI (`free-cli`) is a command-line tool that wraps Claude Code to enable remote control and session sharing. It's part of a three-component system:

1. **free-cli** (this project) - CLI wrapper for Claude Code
2. **handy** - React Native mobile client
3. **handy-server** - Node.js server with Prisma (hosted at https://api.free-servers.com/)
...
FREE_SERVER_URL=http://localhost:3005 ./bin/free.mjs daemon start

# Stop the daemon:
./bin/free.mjs daemon stop

# Check daemon status:
./bin/free.mjs daemon status
```
