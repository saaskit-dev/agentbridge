 ...
 * - connect codex: Store OpenAI API key in Free cloud
 * - connect claude: Store Anthropic API key in Free cloud
 * - connect gemini: Store Gemini API key in Free cloud
...
function showConnectHelp(): void {
  console.log(`
${chalk.bold('free connect')} - Connect AI vendor API keys to Free cloud

${chalk.bold('Usage:')}
  free connect codex        Store your Codex API key in Free cloud
  free connect claude       Store your Anthropic API key in Free cloud
  free connect gemini       Store your Gemini API key in Free cloud
  free connect status       Show connection status for all vendors
  free connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in Free cloud. This enables you to use these services through Free
...
${chalk.bold('Notes:')} 
  • You must be authenticated with Free first (run 'free auth login')
  • API keys are encrypted and stored securely in Free cloud
...
async function handleConnectVendor(
  ...
  console.log(chalk.bold(`\n🔌 Connecting ${displayName} to Free cloud\n`));

  // Check if authenticated
  const credentials = await readCredentials();
  if (!credentials) {
    console.log(chalk.yellow('⚠️  Not authenticated with Free'));
    console.log(chalk.gray('  Please run "free auth login" first'));
    process.exit(1);
  }
...
async function handleConnectStatus(): Promise<void> {
  ...
  for (const vendor of vendors) {
    try {
      ...
      if (token?.oauth) {
        ...
        const expiresAt =
          token.oauth.expires_at ||
          (token.oauth.expires_in ? Date.now() + token.oauth.expires_in * 1000 : null);
  ...
  console.log('');
  console.log(chalk.gray('To connect a vendor, run: free connect <vendor>'));
  console.log(chalk.gray('Example: free connect gemini'));
  console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with Free cloud
 * This ensures the Gemini SDK uses the same account as Free
 */
