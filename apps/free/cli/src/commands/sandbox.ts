...
export async function handleSandboxConfigure(): Promise<void> {
  ...
  const sandboxConfig: SandboxConfig = SandboxConfigSchema.parse({
    enabled: true,
    workspaceRoot:
      scopeMode === 'workspace' ? answers.workspaceRoot || workspaceRootDefault : undefined,
...
export async function handleSandboxStatus(): Promise<void> {
  ...
  if (!config) {
    console.log('Sandbox is not configured. Run `free sandbox configure`.');
    return;
  }
...
export function handleSandboxHelp(): void {
  console.log(`
${chalk.bold('free sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  free sandbox configure      Configure sandbox settings interactively
  free sandbox status         Show current sandbox configuration
  free sandbox disable        Disable sandboxing
  free sandbox help           Show this help
`);
}
