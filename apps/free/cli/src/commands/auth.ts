...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('free auth')} - Authentication management

${chalk.bold('Usage:')}
  free auth login [--force]    Authenticate with Free
  free auth logout             Remove authentication and machine data
  free auth status             Show authentication status
  free auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'free auth login --force' to re-authenticate`));
      return;
  ...
  try {
    ...
    // Auto-start daemon after successful authentication
    if (!(await isDaemonRunningCurrentlyInstalledFreeVersion())) {
      ...
      // Wait for daemon to start (keep trying until success or error)
      let started = false;
      let attempts = 0;
      ...
      console.log(chalk.gray('✓ Daemon already running'));
    }

    // Auto-install system service for auto-start on login
    const { isInstalled } = await import('@/daemon/install');
    try {
      if (!(await isInstalled())) {
        console.log(chalk.gray('\nInstalling system service for auto-start...'));
...
async function handleAuthLogout(): Promise<void> {
  ...
  const freeDir = configuration.freeHomeDir;
  ...
  console.log(chalk.blue('This will log you out of Free'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use Free again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      if (existsSync(freeDir)) {
        rmSync(freeDir, { recursive: true, force: true });
      }
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "free auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "free auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.freeHomeDir}`));
