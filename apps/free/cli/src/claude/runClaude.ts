...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --free-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: freeServer.toolNames.map(toolName => `mcp__free__${toolName}`),
    ...
    // Stop Free MCP server
    freeServer.stop();
    logger.debug('Stopped Free MCP server');
