 ...
 *     │
 *     ├─► generateHookSettingsFile(port) ──► ~/.free/tmp/hooks/session-hook-<pid>.json
 ...
 *
 * ### Triggered By
 * - `free` (fresh start) - new session created
 * - `free --continue` - continues last session (may fork)
 * - `free --resume` - interactive picker, then resume
 * - `free --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 *
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple Free processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between Free process and Claude session.
 */
