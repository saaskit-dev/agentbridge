...
describe('claudeFindLastSession', () => {
  ...
  describe('Multiple sessions scenario', () => {
    it('should find most recent valid session when mixed with invalid ones', () => {
      ...
      writeFileSync(join(testDir, `${invalidId}.jsonl`), JSON.stringify({ type: 'other' }) + '\n');
  ...
  describe('UUID format validation', () => {
    it('should only accept properly formatted UUIDs', () => {
      ...
      writeFileSync(
        join(testDir, 'not-a-uuid.jsonl'),
        JSON.stringify({ uuid: 'msg', type: 'user' }) + '\n'
      );
      writeFileSync(
        join(testDir, '12345678-12-12-12-123456789abc.jsonl'),
        JSON.stringify({ uuid: 'msg', type: 'user' }) + '\n'
      );
      writeFileSync(
        join(testDir, 'XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX.jsonl'),
        JSON.stringify({ uuid: 'msg', type: 'user' }) + '\n'
      );
