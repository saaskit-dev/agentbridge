  ...
  if (message.type === 'assistant') {
    ...
    for (const block of blocks) {
      if (block.type === 'text' && typeof block.text === 'string') {
        envelopes.push(
          createEnvelope('agent', { t: 'text', text: block.text }, { turn: turnId, subagent, time })
        );
        continue;
      }
  ...
  if (message.type === 'user') {
    if (typeof message.message.content === 'string') {
      if (message.isSidechain) {
        ...
        envelopes.push(
          createEnvelope('user', { t: 'text', text: message.message.content }, { time })
        );
      }
    ...
    for (const block of blocks) {
      if (
        ...
        const sessionSubagentForToolResult = getSessionSubagentIdForProviderSubagent(
          state,
          block.tool_use_id
        );
        if (!message.isSidechain) {
