...
export async function claudeLocal(opts: {
  ...
  // We intercept these flags to use free-cli's session storage rather than Claude's default
  //
  // Supported patterns:
  ...
  if (!opts.hookSettingsPath) {
    ...
    if (startFrom) {
      ...
      logger.debug(
        `[ClaudeLocal] User passed ${hasContinueFlag ? '--continue' : '--resume'} flag, session ID will be determined by hook`
      );
    } else {
  ...
  try {
    ...
    await new Promise<void>((r, reject) => {
      ...
      if (!opts.hookSettingsPath) {
        ...
        const hasResumeFlag =
          opts.claudeArgs?.includes('--resume') || opts.claudeArgs?.includes('-r');
        if (startFrom) {
      ...
      if (!claudeCliPath || !existsSync(claudeCliPath)) {
        throw new Error(
          'Claude local launcher not found. Please ensure FREE_PROJECT_ROOT is set correctly for development.'
        );
      }
      ...
      (async () => {
        ...
        if (opts.sandboxConfig?.enabled) {
          if (process.platform === 'win32') {
            ...
            try {
              ...
              const fullCommand = ['node', ...spawnArgs.map(arg => quoteShellArg(arg))].join(' ');
        ...
        child.on('error', error => {
          logger.error('[ClaudeLocal] child process error', error, {
            cwd: opts.path,
            pid: child.pid,
          });
        });
        child.on('exit', async (code, signal) => {
          logger.info('[ClaudeLocal] child process exited', {
            code,
            signal,
            pid: child.pid,
            cwd: opts.path,
