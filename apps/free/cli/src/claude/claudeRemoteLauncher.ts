...
export async function claudeRemoteLauncher(session: Session): Promise<'switch' | 'exit'> {
    ...
    if (message.type === 'assistant') {
      ...
      if (umessage.message.content && Array.isArray(umessage.message.content)) {
        for (const c of umessage.message.content) {
          if (c.type === 'tool_use') {
            logger.debug(
              '[remote]: detected tool use ' + c.id! + ' parent: ' + umessage.parent_tool_use_id
            );
  ...
  try {
    ...
    // See: https://github.com/anthropics/free-cli/issues/143
    ...
    while (!exitReason) {
        ...
        const remoteResult = await claudeRemote({
          ...
          onReady: () => {
            ...
            if (!pending && session.queue.size() === 0) {
              session.api
                .push()
                .sendToAllDevices("It's ready!", `Claude is waiting for your command`, {
      ...
      logger.error('[remote]: launch error with details', {
        ...
        for (const [toolCallId, { parentToolCallId }] of ongoingToolCalls) {
          ...
          if (converted) {
            logger.debug(
              '[remote]: terminating tool call ' + toolCallId + ' parent: ' + parentToolCallId
            );
