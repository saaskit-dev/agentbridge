/**
 * ClaudeBackend — AgentBackend implementation for Claude Code.
 *
...
export class ClaudeBackend implements AgentBackend {
  ...
  private startRemoteMode(opts: AgentStartOpts): void {
    ...
    claudeRemote({
      ...
      onSessionFound: (sessionId: string) => {
        logger.debug('[ClaudeBackend] Claude session found', { sessionId, cwd: opts.cwd, traceId: getProcessTraceContext()?.traceId });
      },
      onMessage: (msg) => {
        if (process.env.APP_ENV === 'development') {
          logger.debug('[ClaudeBackend] raw SDK message', { raw: msg });
        }
    ...
    logger.debug('[ClaudeBackend] remote mode started', {
      cwd: opts.cwd,
  ...
  private startLocalMode(opts: AgentStartOpts): void {
    ...
    logger.debug('[ClaudeBackend] spawning local PTY process', { claudeCliPath, claudeArgs, cwd: opts.cwd, traceId: getProcessTraceContext()?.traceId });
    ...
    logger.debug('[ClaudeBackend] PTY process spawned', { pid: ptyProc.pid, cwd: opts.cwd, traceId: getProcessTraceContext()?.traceId });
    ...
    ptyProc.onData((data: string) => {
      ...
      if (!this.firstPtyDataLogged) {
        ...
        logger.info('[ClaudeBackend] first PTY data after spawn', {
          pid: ptyProc.pid,
          cwd: opts.cwd,
      ...
      if (ptyChunkCount <= 5 || ptyChunkCount % 50 === 0) {
        logger.debug('[ClaudeBackend] pty_data forwarded', { pid: ptyProc.pid, bytes: data.length, chunk: ptyChunkCount });
      }
    ...
    logger.debug('[ClaudeBackend] local mode started', { cwd: opts.cwd, resumeSessionId: opts.resumeSessionId, traceId: getProcessTraceContext()?.traceId });
  }
  ...
  async sendMessage(text: string, permissionMode?: PermissionMode): Promise<void> {
    ...
    if (this.ptyProcess) {
      ...
      logger.debug('[ClaudeBackend] sendMessage (local PTY)', { cwd: this.startOpts.cwd, traceId: getProcessTraceContext()?.traceId, preview: trimmed.slice(0, 100) });
      this.ptyProcess.write(trimmed + '\r');
      return;
    }
    // Remote (SDK) mode
    logger.debug('[ClaudeBackend] sendMessage (remote SDK)', { cwd: this.startOpts.cwd, traceId: getProcessTraceContext()?.traceId, preview: text.slice(0, 100) });
  ...
  resizePty(cols: number, rows: number): void {
    if (this.ptyProcess) {
      logger.debug('[ClaudeBackend] resizePty', { cols, rows, pid: this.ptyProcess.pid });
  ...
  async abort(): Promise<void> {
    logger.debug('[ClaudeBackend] abort — closing inner queue and aborting controller', { cwd: this.startOpts?.cwd, traceId: getProcessTraceContext()?.traceId });
  ...
  async stop(): Promise<void> {
    logger.debug('[ClaudeBackend] stop', { cwd: this.startOpts?.cwd, traceId: getProcessTraceContext()?.traceId });
    this.innerQueue?.close();
    this.abortController.abort();
    ...
    if (this.ptyProcess && this.ptyExitPromise) {
      ...
      if (!result && this.ptyProcess) {
        ...
        logger.warn('[ClaudeBackend] PTY did not exit after SIGTERM, sending SIGKILL', {
