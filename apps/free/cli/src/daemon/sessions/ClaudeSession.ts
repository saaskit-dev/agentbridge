/**
 * ClaudeSession — AgentSession subclass for Claude Code agent.
 *
...
const logger = new Logger('daemon/sessions/ClaudeSession');
...
export class ClaudeSession extends AgentSession<EnhancedMode> {
  ...
  private setClaudeSessionId(sessionId: string) {
    ...
    logger.info('[ClaudeSession] updated resumeSessionId', { sessionId });
  }
  ...
  override async initialize(): Promise<void> {
    ...
    this.hookServer = await startHookServer({
      onSessionHook: (sessionId, _data) => {
        logger.debug('[ClaudeSession] Session hook received', { sessionId });
    ...
    logger.info('[ClaudeSession] HookServer started', {
    ...
    if (this.hookServer && this.hookSettingsFilePath) {
      try {
        ...
        logger.error('[ClaudeSession] Failed to refresh hook settings file', toError(e));
      }
    }
    ...
    if (this.opts.startingMode === 'local') {
        ...
        onMessage: (message) => {
          ...
          if (message.type === 'user') {
            ...
            if (idx !== -1) {
              ...
              logger.debug('[ClaudeSession] Skipping app-originated user message from JSONL (dedup)', { text: jsonlText.slice(0, 100) });
              this.lastStatus = 'working';
              this.session.sendNormalizedMessage({
                role: 'event',
                content: { type: 'status', state: 'working' },
      ...
      logger.info('[ClaudeSession] Session scanner started for local PTY sync', {
        cwd: this.opts.cwd,
  ...
  override requestSwitchToLocal(): void {
    if (this.remoteLegAbort) {
      ...
      logger.info('[ClaudeSession] requestSwitchToLocal — aborting remote leg', {
  ...
  override async run(): Promise<void> {
    ...
    try {
      while (!this.shouldExit) {
        logger.info('[ClaudeSession] loop iteration', { mode });
  ...
  private async runLocalLeg(): Promise<'switch' | 'exit'> {
    ...
    logger.info('[ClaudeSession] local PTY started', {
    ...
    this.messageQueue.setOnMessage(() => {
      if (!switched) {
        switched = true;
        this.switchTimestamp = Date.now();
        logger.info('[ClaudeSession] App message → local→remote switch start');
    ...
    logger.info('[ClaudeSession] local leg ended', { switched });
  ...
  private async runRemoteLeg(): Promise<void> {
    ...
    try {
      await claudeRemote({
        ...
        nextMessage: async () => {
          ...
          if (immediate) {
            logger.info('[ClaudeSession] remote: nextMessage (queued)', {
          ...
          logger.debug('[ClaudeSession] remote: nextMessage blocking...');
          try {
            const item = await this.messageQueue.waitForMessagesAndGetAsString(abortCtrl.signal);
            if (!item) return null;
            logger.info('[ClaudeSession] remote: nextMessage (waited)', {
              sinceStart: Date.now() - remoteLegStart,
        ...
        onReady: () => {
          logger.info('[ClaudeSession] remote SDK ready', {
            sinceStart: Date.now() - remoteLegStart,
          });
        ...
        onSessionFound: (sessionId: string) => {
          logger.debug('[ClaudeSession] remote SDK session found', { sessionId });
          this.setClaudeSessionId(sessionId);
          if (this.session) {
            this.session.updateMetadata((m) => ({ ...m, agentSessionId: sessionId }));
          }
        ...
        onCompletionEvent: (message: string) => {
          logger.info('[ClaudeSession] remote completion', {
            sinceStart: Date.now() - remoteLegStart,
            message,
          });
        },
      ...
      if (!abortCtrl.signal.aborted) {
        logger.error('[ClaudeSession] remote leg error',
          toError(err));
      }
    } finally {
    ...
    this.remoteLegAbort = null;
    logger.info('[ClaudeSession] remote leg ended');
  }
  ...
  override async shutdown(reason: string): Promise<void> {
    try {
      if (this.sessionScanner) {
        ...
        logger.info('[ClaudeSession] Session scanner cleaned up');
      }
      ...
      if (this.hookSettingsFilePath && existsSync(this.hookSettingsFilePath)) {
        try {
          ...
          logger.debug('[ClaudeSession] Failed to delete hook settings file', {
