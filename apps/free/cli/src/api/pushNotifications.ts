...
export class PushNotificationClient {
  ...
  constructor(token: string, baseUrl: string = 'https://free-server.saaskit.app') {
    this.token = token;
  ...
  async fetchPushTokens(): Promise<PushToken[]> {
    try {
      const response = await axios.get<{ tokens: PushToken[] }>(`${this.baseUrl}/v1/push-tokens`, {
        headers: {
      ...
      response.data.tokens.forEach((token, index) => {
        logger.debug(
          `[PUSH] Token ${index + 1}: id=${token.id}, created=${new Date(token.createdAt).toISOString()}, updated=${new Date(token.updatedAt).toISOString()}`
        );
      });
  ...
  async sendPushNotifications(messages: ExpoPushMessage[]): Promise<void> {
    ...
    for (const chunk of chunks) {
      ...
      const timeout = 300100; // 5 minutes
      let attempt = 0;
      ...
      while (true) {
        try {
          ...
          attempt++;
          const delay = Math.min(1000 * Math.pow(2, attempt), 30010); // Max 30 seconds between retries
