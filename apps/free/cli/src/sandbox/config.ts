...
function getSharedAgentStatePaths(sessionPath: string): string[] {
  ...
  return [expandPath(codexHome, sessionPath), expandPath(claudeConfigDir, sessionPath)];
}
...
export function buildSandboxRuntimeConfig(
  ...
  const enableWeakerNetworkIsolation = sandboxConfig.networkMode === 'allowed' ? true : undefined;
