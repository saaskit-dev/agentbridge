...
const sessionPath = '/tmp/free-session';
...
function expectedSharedAgentStatePaths(): string[] {
  ...
  return [...new Set([resolveLikeRuntime(codexHome), resolveLikeRuntime(claudeConfigDir)])];
}
...
describe('buildSandboxRuntimeConfig', () => {
  ...
  it('includes overridden CODEX_HOME and CLAUDE_CONFIG_DIR in allowWrite', () => {
    ...
    try {
      ...
      expect(runtimeConfig.filesystem?.allowWrite).toContain(
        resolve(sessionPath, './custom-claude-config')
      );
    } finally {
