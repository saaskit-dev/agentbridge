 ...
 * @returns true if the prompt contains change_title or free__change_title
 */
export function hasChangeTitleInstruction(prompt: string): boolean {
  return (
    prompt.toLowerCase().includes('change_title') ||
    prompt.toLowerCase().includes('free__change_title')
  );
}
