...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'free connect gemini')
  //
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from Free cloud');
  ...
  // (assigned later after Free server setup)
  ...
  const emitReadyIfIdle = (): boolean => {
    ...
    if (pendingExit) {
      logger.debug('[Gemini] Conversation complete, exiting due to CLI update');
      handleKillSession();
      return false;
    }
  ...
  process.on('SIGTERM', () => {
    logger.debug('[Gemini] Received SIGTERM, waiting for conversation to complete...');
    pendingExit = true;
  ...
  const updateDisplayedModel = (model: string | undefined, saveToConfig: boolean = false) => {
    ...
    logger.debug(
      `[gemini] updateDisplayedModel called: oldModel=${oldModel}, newModel=${model}, saveToConfig=${saveToConfig}`
    );
  ...
  //
  // Start Free MCP server and create Gemini backend
  //
      ...
      switch (msg.type) {
        case 'model-output':
          if (msg.textDelta) {
            ...
            if (!isResponseInProgress) {
              ...
              logger.debug(
                `[gemini] Started new response, first chunk length: ${msg.textDelta.length}`
              );

              // Send streaming text delta if server supports it
              ...
              logger.debug(
                `[gemini] Updated response, chunk length: ${msg.textDelta.length}, total accumulated: ${accumulatedResponse.length + msg.textDelta.length}`
              );
        ...
        case 'status':
          ...
          const statusDetail = msg.detail
            ...
            : '';
          logger.debug(
            `[gemini] Status changed: ${msg.status}${statusDetail ? ` - ${statusDetail}` : ''}`
          );
          ...
          if (msg.status === 'running') {
            ...
            // Send streaming text complete to end streaming on error
            if (serverCapabilities.supportsTextDelta() && currentResponseMessageId) {
              session.sendStreamingTextComplete(currentResponseMessageId, '');
            }
            ...
            if (errorMessage.includes('Authentication required')) {
              errorMessage =
                ...
                `For Google Workspace accounts, run: free gemini project set <project-id>\n` +
                `Or use a different Google account: free connect gemini\n` +
        ...
        case 'tool-call':
          ...
          logger.debug(
            `[gemini] 🔧 Tool call received: ${msg.toolName} (${msg.callId})${isInvestigationTool ? ' [INVESTIGATION]' : ''}`
          );
          if (
            isInvestigationTool &&
            msg.args &&
            typeof msg.args === 'object' &&
            'objective' in msg.args
          ) {
            logger.debug(
              `[gemini] 🔍 Investigation objective: ${String(msg.args.objective).substring(0, 150)}...`
            );
          }
        ...
        case 'tool-result':
          ...
          if (
            ...
            msg.toolName === 'free__change_title'
          ) {
          ...
          const resultSize =
            typeof msg.result === 'string' ? msg.result.length : JSON.stringify(msg.result).length;

          logger.debug(
            `[gemini] ${isError ? '❌' : '✅'} Tool result received: ${msg.toolName} (${msg.callId}) - Size: ${resultSize} bytes${isError ? ' [ERROR]' : ''}`
          );
          ...
          if (isError) {
            ...
            if (resultSize > 1000) {
              logger.debug(
                `[gemini] ✅ Large tool result (${resultSize} bytes) - first 200 chars: ${truncatedResult}`
              );
            }
        ...
        case 'event':
          ...
          if (msg.name === 'thinking') {
            ...
            if (thinkingText) {
              ...
              logger.debug(
                `[gemini] 💭 Thinking chunk received: ${thinkingText.length} chars - Preview: ${thinkingText.substring(0, 100)}...`
              );
  ...
  try {
    ...
    let pending: { message: string; mode: GeminiMode; isolate: boolean; hash: string } | null =
      null;
    ...
    while (!shouldExit) {
      ...
      if (pendingExit) {
        logger.debug(
          '[Gemini] Pending exit (CLI updated), not consuming new messages, waiting for idle...'
        );
      ...
      let message: { message: string; mode: GeminiMode; isolate: boolean; hash: string } | null =
        pending;
      pending = null;
      if (!message) {
        ...
        logger.debug(
          `[gemini] Main loop: received message from queue (length: ${batch.message.length})`
        );
        message = batch;
      }
      ...
      if (wasSessionCreated && currentModeHash && message.hash !== currentModeHash) {
        ...
        if (conversationHistory.hasHistory()) {
          ...
          logger.debug(
            `[Gemini] Will inject conversation history: ${conversationHistory.getSummary()}`
          );
        } else {
        ...
        const modelToUse =
          message.mode?.model === undefined ? undefined : message.mode.model || null;
        ...
        logger.debug(
          `[gemini] Model change - modelToUse=${modelToUse}, actualModel=${actualModel} (from ${backendResult.modelSource})`
        );
      ...
      try {
        if (first || !wasSessionCreated) {
          ...
          if (!geminiBackend) {
            ...
            const actualModel = backendResult.model;
            logger.debug(
              `[gemini] Backend created, model will be: ${actualModel} (from ${backendResult.modelSource})`
            );
            logger.debug(`[gemini] Calling updateDisplayedModel with: ${actualModel}`);
          ...
          if (!acpSessionId) {
            ...
            if (opts.resumeSessionId && geminiBackend.supportsLoadSession?.()) {
              logger.debug('[gemini] Resuming ACP session:', opts.resumeSessionId);
              // Update permission handler with current permission mode before resuming session
              updatePermissionMode(message.mode.permissionMode);
              const { sessionId: resumedSessionId } = await geminiBackend.loadSession!(
              ...
              logger.debug(`[gemini] ACP session resumed: ${acpSessionId}`);
              // Clear resumeSessionId so we don't try to resume again
              opts.resumeSessionId = undefined;
            } else {
              logger.debug('[gemini] Starting new ACP session...');
        ...
        pendingChangeTitle =
          ...
          message.message.includes('free__change_title');
        ...
        if (injectHistoryContext && conversationHistory.hasHistory()) {
          ...
          logger.debug(
            `[gemini] Injected conversation history context (${historyContext.length} chars)`
          );
          // Don't clear history - keep accumulating for future model changes
        }

        logger.debug(
          `[gemini] Sending prompt to Gemini (length: ${promptToSend.length}): ${promptToSend.substring(0, 100)}...`
        );
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            if (isRetryable && attempt < MAX_RETRIES) {
              logger.debug(
                `[gemini] Retryable error on attempt ${attempt}/${MAX_RETRIES}: ${errorDetails}`
              );
              messageBuffer.addMessage(
                `Gemini returned empty response, retrying (${attempt}/${MAX_RETRIES})...`,
                'status'
              );
        ...
        if (isAbortError) {
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            // Check for quota/capacity exceeded error
              ...
              // Extract reset time from error message like "Your quota will reset after 3h20m35s."
              const resetTimeMatch = (errorDetails + errorMessage + errorString).match(
                /reset after (\d+h)?(\d+m)?(\d+s)?/i
              );
              let resetTimeMsg = '';
              if (resetTimeMatch) {
                const parts = resetTimeMatch.slice(1).filter(Boolean).join('');
        ...
        if (accumulatedResponse.trim()) {
          ...
          logger.debug(
            `[gemini] Sending complete message to mobile (length: ${finalMessageText.length}): ${finalMessageText.substring(0, 100)}...`
          );
          session.sendAgentMessage('gemini', messagePayload);

          // Send streaming text complete if server supports it
          if (serverCapabilities.supportsTextDelta() && currentResponseMessageId) {
            session.sendStreamingTextComplete(currentResponseMessageId, finalMessageText);
            currentResponseMessageId = null;
          }
