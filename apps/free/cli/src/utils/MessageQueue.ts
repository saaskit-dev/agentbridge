...
export class MessageQueue implements AsyncIterable<SDKUserMessage> {
  ...
  push(message: string): void {
    ...
    logger.debug(
      `[MessageQueue] push() called. Waiters: ${this.waiters.length}, Queue size before: ${this.queue.length}`
    );
    ...
    logger.debug(
      `[MessageQueue] push() completed. Waiters: ${this.waiters.length}, Queue size after: ${this.queue.length}`
    );
  }
    ...
    return new Promise(resolve => {
      ...
      this.closePromise?.then(() => {
        ...
        if (index !== -1) {
          ...
          logger.debug(
            `[MessageQueue] waitForNext() waiter removed due to close. Remaining waiters: ${this.waiters.length}`
          );
          resolve(undefined);
        }
      });
