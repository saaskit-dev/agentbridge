 ...
 * Ensures consistent tmux handling across free-cli with proper session naming
 */
...
export function parseTmuxSessionIdentifier(identifier: string): TmuxSessionIdentifier {
  ...
  if (parts.length > 1) {
    ...
    if (result.window && !/^[a-zA-Z0-9._-]+$/.test(result.window)) {
      throw new TmuxSessionIdentifierError(
        `Invalid window name: "${result.window}". Only alphanumeric characters, dots, hyphens, and underscores are allowed.`
      );
    }
...
export function extractSessionAndWindow(
  tmuxOutput: string
...
export class TmuxUtilities {
  ...
  public static readonly DEFAULT_SESSION_NAME = 'free';
  ...
  async spawnInTmux(
    ...
    try {
      ...
      // - undefined: Use first existing session or create "free"
      // - empty string: Use first existing session or create "free"
      // - specific name: Use that session (create if doesn't exist)
      let sessionName =
        options.sessionName !== undefined && options.sessionName !== ''
          ? options.sessionName
          : null;
      ...
      if (!sessionName) {
        ...
        if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
          ...
          // No sessions exist, create "free"
          sessionName = 'free';
          logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
        }
      }

      const windowName = options.windowName || `free-${Date.now()}`;
      ...
      if (env && Object.keys(env).length > 0) {
        ...
        logger.debug(
          `[TMUX] Setting ${Object.keys(env).length} environment variables in tmux window`
        );
      }
      ...
      logger.debug(
        `[TMUX] Spawned command in tmux session ${sessionName}, window ${windowName}, PID ${panePid}`
      );
 ...
 */
export function validateTmuxSessionIdentifier(identifier: string): {
  valid: boolean;
  error?: string;
} {
  try {
