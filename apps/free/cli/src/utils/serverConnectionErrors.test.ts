...
vi.mock('axios', () => ({
  ...
  isAxiosError: (e: unknown) => {
    return (
      e !== null && typeof e === 'object' && 'isAxiosError' in e && (e as any).isAxiosError === true
    );
  },
}));
  ...
  describe('error handling', () => {
    it('should stop retrying on 401 authentication error', async () => {
      ...
      expect(onNotify).toHaveBeenCalledWith(
        '❌ Authentication failed. Please re-authenticate with `free auth`.'
      );
      expect(onReconnected).not.toHaveBeenCalled();
    ...
    it('should NOT stop retrying on 403 forbidden (not auth failure)', async () => {
      ...
      expect(onNotify).not.toHaveBeenCalledWith(expect.stringContaining('Authentication failed'));
      expect(onNotify).toHaveBeenCalledWith('✅ Reconnected! Session syncing in background.');
  ...
  it('should print offline warning with unified format', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        '⚠️  Free server unreachable, offline mode with auto-reconnect enabled'
      )
    );
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Server connection failed'));
