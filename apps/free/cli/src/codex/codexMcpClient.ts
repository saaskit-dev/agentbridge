...
export class CodexMcpClient {
  ...
  constructor(sandboxConfig?: SandboxConfig) {
    ...
    this.client = new Client(
      { name: 'free-codex-client', version: '1.0.0' },
  ...
  async connect(): Promise<void> {
    ...
    if (this.sandboxConfig?.enabled) {
      if (process.platform === 'win32') {
        ...
        try {
          ...
          logger.warn(
            '[CodexMCP] Failed to initialize sandbox; continuing without sandbox.',
            error
          );
    ...
    try {
      const transportEnv = Object.keys(process.env).reduce(
        (acc, key) => {
  ...
  private registerPermissionHandlers(): void {
    ...
    this.client.setRequestHandler(ElicitRequestSchema, async request => {
      ...
      try {
        ...
        const result = await this.permissionHandler.handleToolCall(params.codex_call_id, toolName, {
  ...
  async continueSession(
    ...
    if (!this.conversationId) {
      ...
      logger.debug(
        '[CodexMCP] conversationId missing, defaulting to sessionId:',
        this.conversationId
      );
    }
