...
export async function runCodex(opts: {
  ...
  function logActiveHandles(tag: string) {
    ...
    const handles =
      typeof anyProc._getActiveHandles === 'function' ? anyProc._getActiveHandles() : [];
    const requests =
      typeof anyProc._getActiveRequests === 'function' ? anyProc._getActiveRequests() : [];
  ...
  client.setHandler(msg => {
    ...
    if (msg.type === 'patch_apply_begin') {
      ...
      const { auto_approved, changes } = msg;
  ...
  try {
    ...
    let pending: { message: string; mode: EnhancedMode; isolate: boolean; hash: string } | null =
      null;
    ...
    while (!shouldExit) {
      ...
      if (pendingExit) {
        logger.debug(
          '[Codex] Pending exit (CLI updated), not consuming new messages, waiting for idle...'
        );
      ...
      let message: { message: string; mode: EnhancedMode; isolate: boolean; hash: string } | null =
        pending;
      pending = null;
      if (!message) {
    ...
    // Stop Free MCP server
    logger.debug('[codex]: freeServer.stop');
    freeServer.stop();
