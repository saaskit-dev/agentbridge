...
export async function runDoctorCommand(filter?: 'all' | 'daemon'): Promise<void> {
    ...
    if (filter === 'all') {
        ...
        console.log(`FREE_HOME_DIR: ${env.FREE_HOME_DIR ? chalk.green(env.FREE_HOME_DIR) : chalk.gray('not set')}`);
        console.log(`FREE_SERVER_URL: ${env.FREE_SERVER_URL ? chalk.green(env.FREE_SERVER_URL) : chalk.gray('not set')}`);
