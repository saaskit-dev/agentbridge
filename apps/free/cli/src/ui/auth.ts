...
async function doMobileAuth(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
  ...
  console.log('Scan this QR code with your Free mobile app:\n');

  const authUrl = 'free://terminal?' + encodeBase64Url(keypair.publicKey);
...
async function doWebAuth(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
  ...
  // someone running free inside a devcontainer that they saw the
    ...
    while (!cancelled) {
      try {
        ...
        if (response.data.state === 'authorized') {
          ...
          const r = decodeBase64(response.data.response);
          const decrypted = decryptWithEphemeralKey(r, keypair.secretKey);
          if (decrypted) {
