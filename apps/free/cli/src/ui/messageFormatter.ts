  ...
  switch (message.type) {
    case 'system': {
      ...
      if (sysMsg.subtype === 'init') {
        ...
        console.log(
          chalk.blue.bold('🚀 Session initialized:'),
          chalk.cyan(sysMsg.session_id?.replace(/-/g, ''))
        );
        ...
        // Handle array content (can contain text blocks and tool result blocks)
          ...
          for (const block of content) {
            if (block.type === 'text') {
              ...
              console.log(
                chalk.green.bold('\n✅ Tool Result:'),
                chalk.gray(`(Tool ID: ${block.tool_use_id})`)
              );
              if (block.content) {
                const outputStr =
          ...
          if (block.type === 'text') {
            ...
            if (block.input) {
              ...
              if (inputStr.length > maxLength) {
                console.log(
                  chalk.gray('Input:'),
                  inputStr.substring(0, maxLength) + chalk.gray('\n... (truncated)')
                );
              } else {
      ...
      if (resultMsg.subtype === 'success') {
        ...
        if (resultMsg.usage) {
          ...
          if (resultMsg.usage.cache_read_input_tokens) {
            console.log(
              chalk.gray(`  • Cache read tokens: ${resultMsg.usage.cache_read_input_tokens}`)
            );
          }
          if (resultMsg.usage.cache_creation_input_tokens) {
            console.log(
              chalk.gray(
                `  • Cache creation tokens: ${resultMsg.usage.cache_creation_input_tokens}`
              )
            );
          }
