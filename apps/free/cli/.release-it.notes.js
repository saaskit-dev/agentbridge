...
async function generateReleaseNotes() {
  try {
    ...
    let gitLog;
    try {
      gitLog = execSync(`git log ${commitRange} --pretty=format:"%h - %s (%an, %ar)" --no-merges`, {
        encoding: 'utf8',
      });
    } catch (error) {
      // Fallback to recent commits if tag doesn't exist
      console.error(`Tag ${fromTag ?? '(none)'} not found, using recent commits instead`);
      gitLog = execSync(`git log -10 --pretty=format:"%h - %s (%an, %ar)" --no-merges`, {
        encoding: 'utf8',
      });
    }
    ...
    const prompt = `Please analyze these git commits and generate professional release notes for version ${toVersion} of the Free CLI tool (a Claude Code session sharing CLI).

The release should cover commits since the latest stable tag (vX.Y.Z): ${fromTag ?? '(none)'}.
...
Do not include any preamble or explanations, just return the markdown release notes.`;
    ...
    const releaseNotes = execFileSync('claude', ['--add-dir', '.', '--print', prompt], {
      encoding: 'utf8',
