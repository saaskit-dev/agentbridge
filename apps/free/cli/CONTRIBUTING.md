...
### Global `free-dev` Command

Create a global `free-dev` command that runs your local development build:

```bash
yarn link:dev      # Create free-dev symlink
yarn unlink:dev    # Remove free-dev symlink
```
...
3. Now `cd` into the directory automatically sets `FREE_VARIANT=dev`!
...
4. **Different accounts** - Use different Free accounts for dev/stable if needed
5. **Check logs** - Logs are separated: `~/.free/logs/` vs `~/.free-dev/logs/`
...
The system uses the built-in `FREE_HOME_DIR` environment variable to separate data:

- **Stable scripts** set: `FREE_HOME_DIR=~/.free`
- **Dev scripts** set: `FREE_HOME_DIR=~/.free-dev`

Everything else (auth, sessions, logs, daemon) automatically follows the `FREE_HOME_DIR` setting.

Cross-platform via Node.js - works identically on Windows, macOS, and Linux!

## Testing Profile Sync Between GUI and CLI

Profile synchronization ensures AI backend configurations created in the Free mobile/web GUI work seamlessly with the CLI daemon.
...
2. **Verify CLI receives profile:**
   ...
   # Check daemon logs
   tail -f ~/.free-dev/logs/*.log | grep -i profile
   ```
...
# or directly:
yarn workspace @free/cli release
```
...
**Note:** `free-dev` is intentionally excluded from the npm package - it's for local development only.
