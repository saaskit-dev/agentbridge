#!/usr/bin/env bash
# Free CLI - Test --continue Flag Fix
# usage: ./scripts/free-cli-test-continue-fix.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
CLI_DIR="$ROOT_DIR/apps/free/cli"
...
echo "  free --continue \"test continuation\""
echo
echo "To check logs:"
echo "  tail -50 ~/.free/logs/\$(ls -t ~/.free/logs/ | head -1) | grep -E '(session|continue|resume)'"
