...
4. **Legacy Backend 处置与设计一致**：`ClaudeBackend`（SDK/PTY）保留作为 `--agent claude` 默认模式，`CodexBackend`（MCP）保留但已非主路径

### 实现文件

| 文件 | 用途 |
|------|------|
| `cli/src/backends/claude-acp/ClaudeAcpBackend.ts` | Claude ACP backend |
| `cli/src/backends/claude-acp/mapClaudeAcpRawToNormalized.ts` | Claude ACP 消息映射 |
| `cli/src/backends/codex-acp/CodexAcpBackend.ts` | Codex ACP backend |
| `cli/src/backends/codex-acp/mapCodexAcpRawToNormalized.ts` | Codex ACP 消息映射 |
