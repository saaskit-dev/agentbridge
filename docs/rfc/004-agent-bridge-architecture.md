...
新 daemon 启动
  ...
  └─ readAllPersistedSessions()
       │
       ├─ agentType 支持 loadSession（claude-acp）？
       │    └─ spawn 新 agent + loadSession(resumeSessionId) → 恢复对话上下文
       │
       └─ 不支持 loadSession（gemini, opencode, cursor, codex）？
            └─ spawn 新 agent + startSession() → 新建 session（对话历史丢失）
```
...
新 daemon 启动（用户手动或 watchdog）
  │
  └─ 同上：读持久化 → loadSession 或 startSession
```
...
| **不支持 loadSession 的 agent 重启后丢失对话** | Gemini、OpenCode、Cursor 重启后新建 session，历史聊天记录消失 |
...
`loadSession` 是运行时从 ACP agent 的 `init()` 响应中动态发现的（`agentCapabilities.loadSession`），不是按 agent 类型 hardcode。目前**只有 claude-code 上报此能力**，Cursor / Gemini / OpenCode / Codex 均不支持。

这意味着：这些 agent 在 daemon crash 或重启后，**现在就已经**会丢失整个对话历史（重建为新 session）。这不是未来的问题，而是当前已存在的限制。

### 升级决策框架

**可以继续接受当前方案（不升级 Bridge）的条件——全部满足时：**
- daemon crash 是偶发事件，用户可接受"重连后重建 session"的体验
- 版本更新时优雅重启覆盖了绝大多数中断场景
- 对话历史通过服务端消息缓存（`lastSeq` + SQLite）可以在新 session 中浏览，不完全丢失

**应该升级 Bridge 的信号——出现任一时评估：**
- 生产 crash 频率上升，用户频繁反馈"重连后 session 没了"
- 出现需要持续数小时的后台 agent 任务（用户不在线时 agent 继续跑）
- 产品上需要"进程级会话持久化"语义（类 tmux，重启终端 agent 还在）
...
| Daemon crash（Cursor/Gemini/OpenCode/Codex）| ❌ 对话历史丢失，新建 session（这些 agent 均不支持 loadSession，现状如此）| ✅ re-attach，完全透明 |
