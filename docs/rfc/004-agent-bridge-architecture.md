...
| 条件 | 说明 |
|------|------|
| **生产 crash 频率明显上升** | daemon crash 导致用户频繁反馈对话中断、数据丢失 |
| **Gemini / OpenCode / Cursor 用量超过 Claude** | 这些 agent 不支持 loadSession，重启后丢失完整对话历史，成为主要痛点 |
| **长时间运行 turn 成为常态** | 如深度代码重构任务持续 30min+，5 分钟等待超时不够用 |
| **多 daemon 并发场景出现** | 未来如果支持多工作区/多用户同时跑 daemon，Bridge 隔离性更好 |
| **用户明确要求"后台持续运行"语义** | agent 需要在用户完全离线时继续执行（类 tmux 会话）|

### 不需要升级的条件

- daemon 版本更新是主要重启场景（已有优雅重启解决）
- Claude ACP 是主力 agent（loadSession 覆盖恢复场景）
- crash 是偶发事件，用户可接受重建代价
...
| Daemon crash（其他 agent）| ❌ 对话历史丢失，新建 session | ✅ re-attach，完全透明 |
