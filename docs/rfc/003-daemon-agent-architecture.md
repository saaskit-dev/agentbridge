...
function setupOfflineReconnection(opts: SetupOfflineReconnectionOptions): SetupOfflineReconnectionResult
```
...
export type NormalizedAgentContent =
  ...
  | { type: 'tool-result'; tool_use_id: string; content: unknown; is_error: boolean; uuid: string; parentUUID: string | null; permissions?: PermissionResult }
...
export abstract class AgentSession<TMode> {
  ...
  handleSigterm(): void { this.pendingExit = true; }
...
export type SessionLifecycleState =
  | 'initializing' | 'ready' | 'working' | 'idle' | 'archived';
...
export class IPCServer {
  ...
  async start(socketPath: string): Promise<void> {
    ...
    try { fs.unlinkSync(socketPath); } catch { /* 不存在则忽略 */ }
    ...
    switch (msg.type) {
      case 'attach': {
        ...
        socket.write(JSON.stringify({
          type: 'history', sessionId: msg.sessionId, msgs: history
        }) + '\n');
        break;
      }
      case 'detach': {
      ...
      case 'spawn_session': {
        ...
        this.onSpawnSession(msg.opts).then(result => {
          socket.write(JSON.stringify({
            type: 'spawn_result',
          ...
          socket.write(JSON.stringify({
            type: 'spawn_result', sessionId: '', success: false, error: String(err),
          }) + '\n');
        });
        break;
      }
      case 'abort': {
        ...
        session?.abort().catch(err => {
          logger.error('[IPCServer] abort failed', { sessionId: msg.sessionId, error: String(err) });
        });
        break;
      }
      case 'list_sessions': {
  ...
  send(msg: IPCClientMessage): void {
    ...
    if (msg.type === 'send_input') {
      if (this.pendingSendInputs.length < this.PENDING_SEND_INPUT_LIMIT) {
        ...
        logger.warn('[IPCClient] send_input buffered: daemon reconnecting, will resend on reconnect', {
...
export class ClaudeSession extends AgentSession<EnhancedMode> {
  ...
  createBackend(): AgentBackend { return new ClaudeBackend(); }

  createModeHasher() {
    return (mode: EnhancedMode) => JSON.stringify({
  ...
  protected buildInitialState(): AgentState { return {}; }
}
```
...
export class ClaudeBackend implements AgentBackend {
  ...
  async start(opts: AgentStartOpts): Promise<void> {
    ...
    this.messageQueue = new MessageQueue2<EnhancedMode>(
      (mode) => JSON.stringify({ permissionMode: mode.permissionMode, model: mode.model })
    );

    // fire-and-forget，加错误处理防止 unhandled rejection
    const run = opts.startingMode === 'local'
      ? this.runLocal(opts)
      : this.runRemote(opts);
    run.catch(err => {
  ...
  private async runLocal(opts: AgentStartOpts): Promise<void> {
    ...
    const id = createId();
    if (result.type === 'exit') {
      this.output.push({ id, localId: null, createdAt: Date.now(), isSidechain: false,
        role: 'event', content: { type: 'message', message: 'session ended' } });
    } else {
      this.output.push({ id, localId: null, createdAt: Date.now(), isSidechain: false,
        role: 'event', content: { type: 'switch', mode: 'remote' } });
    }
    this.output.end();
  }
  ...
  private async runRemote(opts: AgentStartOpts): Promise<void> {
    ...
    if (result === 'exit') {
      ...
      this.output.push({ id, localId: null, createdAt: Date.now(), isSidechain: false,
        role: 'event', content: { type: 'switch', mode: 'local' } });
    }
    this.output.end();
  }
...
export class CodexSession extends AgentSession<EnhancedMode> {
  ...
  createBackend(): AgentBackend { return new CodexBackend(); }
  createModeHasher() {
...
export class CodexBackend implements AgentBackend {
  ...
  async abort(): Promise<void> { await this.client.abort(); }
  async stop(): Promise<void> { await this.client.close(); this.output.end(); }
}
```
...
export function mapCodexRawToNormalized(
  ...
  if (raw.type === 'message') {
    return {
      id, localId: null, createdAt, isSidechain: false,
      role: 'agent',
  ...
  if (raw.type === 'tool-call') {
    return {
      id, localId: null, createdAt, isSidechain: false,
      role: 'agent',
      content: [{ type: 'tool-call', id: raw.callId, name: raw.name, input: raw.input, description: null, uuid: raw.id, parentUUID: null }],
    };
  }
  if (raw.type === 'tool-call-result') {
    return {
      id, localId: null, createdAt, isSidechain: false,
      role: 'agent',
      content: [{ type: 'tool-result', tool_use_id: raw.callId, content: raw.output, is_error: false, uuid: raw.id, parentUUID: null }],
    };
  }
  return null;
}
...
async function loadDaemonCredential(options: SpawnSessionOptions): Promise<Credentials> {
  ...
  const token = options.environmentVariables?.ANTHROPIC_AUTH_TOKEN
    ?? options.token;
...
```typescript
// 1. 定义 mode 类型
interface NewAgentMode { model: string }
...
export class NewAgentBackend implements AgentBackend {
  ...
  async start(opts: AgentStartOpts): Promise<void> { /* 启动 SDK/MCP/ACP */ }
  async sendMessage(text: string): Promise<void>   { /* 发消息 */ }
  async abort(): Promise<void>                      { /* 中止 */ }
  async stop(): Promise<void>                       { this.output.end(); }
}

// 3. 实现 session（约 15 行）
export class NewAgentSession extends AgentSession<NewAgentMode> {
  createBackend() { return new NewAgentBackend(); }
  createModeHasher() { return (m: NewAgentMode) => m.model; }
  protected buildMetadata(): Metadata {
