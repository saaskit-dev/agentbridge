...
## .github/workflows/release-desktop.yml / scripts/create-macos-dmg.sh — 自托管 macOS 发布避开 Tauri 内置 DMG 脚本，改为独立 `hdiutil` 打包并在打包前重做 ad-hoc 签名
...
- macOS 发布 job 先只让 Tauri 产出稳定的 `app` bundle
- 保留 updater 所需的 `.app.tar.gz` / 签名产物
- 在生成 DMG 前对 `.app` 再执行一次 `codesign --force --deep --sign -`
- 后续改用独立 `hdiutil create -srcfolder` 脚本生成简化版 DMG
- 再把该 DMG 作为 release asset 单独上传
