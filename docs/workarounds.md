...
## apps/free/app/sources/components/MultiTextInput.web.tsx — Web 端输入框停止“直接改 DOM + 手工派发 input 事件”，规避 macOS WebKit 输入法崩溃路径

**问题**：桌面端（Tauri/WKWebView）在 macOS 输入法组合输入（IME）路径上，偶发崩溃于
`NSTextInputContext` / `WebKit::WebPage::HasMarkedText`，并在系统日志中出现
`*** CFEqual() called with NULL second argument ***`。原实现会在自动补全等场景中直接改 textarea DOM 值并手工派发 `input` 事件，该模式可能与 WebKit 组合态同步冲突。

**触发条件**：

- 桌面端 WebView（macOS）
- 输入框处于中文/日文等 IME 组合输入相关路径
- 自动补全或快捷键分支调用 `setTextAndSelection`
- 同时发生“直接 DOM 赋值 + 手工 input 事件”重入

**修复内容**：

- `setTextAndSelection` 改为通过 React 受控更新：调用 `onChangeText(text)`，不再直接写 `textarea.value`
- 移除手工 `dispatchEvent(new Event('input'))`
- 光标移动改为下一帧 `setSelectionRange`，并保留组合态保护（`isComposing` 时不执行）

**上游**：属于 macOS WebKit/输入法组合态与前端强制 DOM 同步策略的兼容性问题，当前改动是防御性绕行。

**删除条件**：确认升级后的 WebKit/Tauri 组合在同样输入法场景长期稳定，且回归验证显示可安全恢复“直接 DOM 同步”策略时可移除。
